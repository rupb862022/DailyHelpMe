...
import { myRequests, acceptVol, denyVol, deleteRequest } from '../../FetchCalls/profileAPI';
...
const MyRequests = ({ navigation }) => {
  ...
  const [indexOfCopyText, setIndexOfCopyText] = useState(0);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (indexOfCopyText != 0)
        setIndexOfCopyText(0);
    }, 3000)
    ...
    <View style={{ width: screenWidth, }}>
      <View
        ...
        <TouchableOpacity style={{ width: "75%", alignItems: 'flex-start' }} onPress={() => {
          ...
          <TouchableOpacity style={[styles.row, styles.center]} onPress={() => {
            copyToClipboard(ReqChose.Link);
            setIndexOfCopyText(ReqChose.RequestCode);
          }}>
            <FontAwesome5 name="copy" size={14} color="#F8B11C" />
            <Text style={{ fontSize: 12, marginLeft: 5 }}>{indexOfCopyText == ReqChose.RequestCode ? "קוד בקשה הועתק" : "העתקת קוד בקשה"}</Text>
          </TouchableOpacity>
        </TouchableOpacity>
          ...
          return (
            <View style={{ width: screenWidth * 0.82, alignSelf: 'center', marginBottom: 5, marginTop: 5 }}>
              ...
              <View style={[styles.row, { justifyContent: 'space-between' }]}>
                ...
                <View style={styles.row}>
                <AntDesign name="clockcircleo" size={20} color="#F8B11C" style={{marginRight:5}}/>
      ...
      <View style={{
        ...
        <TouchableOpacity onPress={() => {
          ...
          <View style={{ width: "80%", alignItems: 'flex-start' }}>
            ...
            <TouchableOpacity style={[styles.row, styles.center]} onPress={() => {
              setIndexOfCopyText(item.RequestCode);
              copyToClipboard(item.Link)
            }}>
              <FontAwesome5 name="copy" size={14} color="#F8B11C" />
              <Text style={{ fontSize: 12, marginLeft: 5 }}>{indexOfCopyText == item.RequestCode ? "קוד בקשה הועתק" : "העתקת קוד בקשה"}</Text>
            </TouchableOpacity>
          </View>
          <View style={{ width: "25%", marginTop: 5, }}>
            <View style={[styles.row, { justifyContent: 'space-around', marginBottom: 12 }]}>
              ...
              <TouchableOpacity onPress={() => { DeleteRequest(item); setReqChose(item) }}>
                <FontAwesome5 name="trash" size={18} color="#F8B11C" />
              </TouchableOpacity>
            </View>
            {item.PrivateRequest &&
  ...
  if (loading) {
    ...
    return (
      <View>
        ...
        <CustomDialog visible={showMissionSigned} setClose={setShowMissionSigned} color={'red'} title="זהירות" content={
          <FlatList
            ...
            ListFooterComponent={
              <View style={[styles.row, styles.center, { width: "80%", marginTop: 10, alignSelf: 'center' }]}>
                <TouchableOpacity style={{ alignSelf: 'center', marginTop: 10, }} onPress={() => setShowMissionSigned(false)}>
                  <Text style={{ fontWeight: 'bold' }}> חזור </Text>
                </TouchableOpacity>
                <TouchableOpacity style={{ alignSelf: 'center', marginTop: 10, }} onPress={() => {
                  RequestDeleteApproved(ReqChose.RequestCode)
                  setShowMissionSigned(false)
                }}>
                  <Text style={{ fontWeight: 'bold' }}> המשך </Text>
                </TouchableOpacity>
              </View>
            }
            listKey={"lists4"}
        ...
        <FlatList
          style={{ width: screenWidth, height: screenHeight * 0.62 }}
